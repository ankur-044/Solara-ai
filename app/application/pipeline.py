...
# Updated to use the clients you just provided
...
async def run_pipeline(data_request):
    ...
    try:
        # =========================
        # STEP 1: Get Coordinates (Fixes the Tuple/List crash)
        # =========================
        # get_lat_lon returns (lat, lng) as a tuple
        lat, lon = await get_lat_lon(city)
        
        if lat is None or lon is None:
            raise Exception("Geocoding failed to find coordinates")

        # =========================
        # STEP 2: Real-time Weather (Tomorrow.io)
        # =========================
        weather = await get_tomorrow_weather(lat, lon)
        
        # Extract values from Tomorrow.io response
        temp = weather.get("temp", 25)
        cloud = weather.get("cloud", 0)
        uv_index = weather.get("uv", 0)
        humidity = 50 # Default if Tomorrow.io doesn't provide in snippet
        ...
        for entry in forecast_list[:8]:
            ...
            # CRITICAL: Keys 't' and 'v' match Dashboard.jsx Recharts
            ghi_forecast.append({
                "t": time_label,
        ...
        # =========================
        # Pass the forecast list to generate windows
        windows = generate_solar_windows(forecast_list)
        
        # Pass real values to analysis
        analysis = generate_analysis(irradiance, cloud, humidity)
        
        # Pass windows and irradiance to your provided optimizer
        device_plan = optimize_devices(windows, irradiance)

        # =========================
        # FINAL CLEAN RESPONSE (Matches Frontend Requirements)
        # =========================
        return {
            "location": city.upper(),
            "lat": lat,
            ...
            "aod": 0.14, # Static or calculate if available
